jQuery(document).ready(($) => {
  ...
  var popupData = category404RedirectorData.popup || {}
  ...
  function showPopup() {
    // Check if popup is enabled or forced
    ...
    var html = `
      <div id="c404r-popup-overlay" class="c404r-popup-overlay">
        <div class="c404r-popup c404r-title-${titleFontSize} c404r-subtitle-${subtitleFontSize} c404r-description-${descriptionFontSize} c404r-features-${featuresFontSize} c404r-button-${buttonFontSize} c404r-footer-${footerFontSize} c404r-title-mobile-${titleFontSizeMobile} c404r-subtitle-mobile-${subtitleFontSizeMobile} c404r-description-mobile-${descriptionFontSizeMobile} c404r-features-mobile-${featuresFontSizeMobile} c404r-button-mobile-${buttonFontSizeMobile} c404r-footer-mobile-${footerFontSizeMobile}">
          <div class="c404r-top-bar" style="background-color: ${subtitleBackgroundColor}; width: 100%;"></div>
  ...
  function initPopup() {
    ...
    var isPaused = false
    var $countdown = $(".c404r-countdown")
    var $topBar = $(".c404r-top-bar")
    var initialWidth = 100

    // Start countdown
    if (countdownSeconds > 0) {
      $countdown.text(countdownSeconds + "s")
      $topBar.css("width", initialWidth + "%")
      ...
      countdownInterval = setInterval(() => {
        if (!isPaused) {
          ...
          var percentage = (countdownSeconds / popupData.countdownSeconds) * 100
          $topBar.css("width", percentage + "%")
    ...
    $(".c404r-pause-btn").on("click", function () {
      isPaused = !isPaused
    ...
    $(".c404r-close-btn").on("click", () => {
      closePopup()
    })

    // Handle back button click
    $(".c404r-back-btn").on("click", () => {
      closePopup()
      window.history.back()
    })
  }
  ...
  function handleLinkClick(e) {
    ...
    // Handle empty href or no href - show popup and redirect to default link
    if (!href || href === "") {
      console.log("Empty link detected - Showing popup and redirecting to: " + defaultLink)
      e.preventDefault()
      e.stopPropagation()
    ...
    // Handle hash links - show popup and redirect to default link
    if (
      href === "#" ||
      href === "#respond" ||
      (href.startsWith("#") && href.length > 1 && !href.startsWith("#comment"))
    ) {
      console.log("Hash link detected: " + href + " - Showing popup and redirecting to: " + defaultLink)
      e.preventDefault()
      e.stopPropagation()
    ...
    if (href.indexOf("javascript:void(0)") !== -1 || href.indexOf("javascript:;") !== -1) {
      console.log("JavaScript void link detected - Showing popup and redirecting to: " + defaultLink)
      e.preventDefault()
      e.stopPropagation()
    ...
    $.ajax({
      ...
      success: (response) => {
        if (response.success) {
          if (response.data.exists) {
            ...
            console.log("URL does not exist, showing popup and redirecting to: " + defaultLink)
          ...
          console.log("AJAX error, showing popup and proceeding to original link: " + fullUrl)
      ...
      error: (xhr, status, error) => {
        ...
        console.log("Showing popup and proceeding to original link: " + fullUrl)
  ...
  // If force popup is enabled, show popup on page load after a short delay
  if (forcePopup) {
    setTimeout(() => {
      showPopup()
    }, 1000)
